...
def predicted_expenses(data,par):
    ...
    for i in range(obs_num):
        ...
        pred_expenses[i] = mc_assets*data[i,par.data_asset_q_index] + par.param_vec[par.par_fc_index]
...
def gradient_pred_exp(data,par):
    obs_num = data.shape[0]
    pred_expenses = np.zeros(obs_num)
    d_exp = np.zeros((obs_num,len(par.param_vec)))
    for i in range(obs_num):
        # Deposits and Assets
        ...
        d_exp[i,par.par_fc_index] = 1

        # Cost Prediction
        mc_assets = (spread)*(1-1/par.param_vec[par.par_dep_index])
        pred_expenses[i] = mc_assets*assets + par.param_vec[par.par_fc_index]
...
def hessian_pred_exp(data,par):
    ...
    pred_expenses = np.zeros(obs_num)
    d_exp = np.zeros((obs_num,len(par.param_vec)))
    d2_exp = np.zeros((obs_num,len(par.param_vec),len(par.param_vec)))
    for i in range(obs_num):
        # Deposits and Assets
        spread = data[i,par.data_asset_p_index] + data[i,par.data_dep_p_index]
        assets = data[i,par.data_asset_q_index]
        ...
        d2_exp[i,par.par_fc_index,par.par_fc_index] = 0
