...
def cost_moments_derivatives(data,par):
    ...
    for k in range(K):
        spec = par.dem_spec_list[k]
        X_beta = data[spec['index'],:]
        X_beta = X_beta[:,spec['ind_var']]
        X_gamma = data[spec['index'],:]
        X_gamma = X_gamma[:,par.cost_spec['ind_var']]
        # print(np.ix_([k],par.cost_spec['param'],spec['param']))
